...
RUN export DEBIAN_FRONTEND=noninteractive && \
    ...
    apt-get update && \
    apt-get install -y --no-install-recommends nodejs p11-kit libasound2t64 libx11-xcb1 libdbus-glib-1-2 libxt6 libgtk-3-0 \
            ffmpeg dumb-init && \
...
RUN PLAYWRIGHT_SKIP_BROWSER_DOWNLOAD=1 npm install playwright@$PLAYWRIGHT_VERSION &&  \
    ...
    # make firefox using system-wide trust store
    FIREFOX_REVISION=$(jq -r '.browsers[] | select(.name == "firefox") | .revision' <node_modules/playwright-core/browsers.json) && \
    ln -sf /usr/lib/x86_64-linux-gnu/pkcs11/p11-kit-trust.so .cache/ms-playwright/firefox-${FIREFOX_REVISION}/firefox/libnssckbi.so
