...
func (m *Meta) updatePW() error {
	...
	for _, tag := range releaseTags {
		...
		changed = m.updateTags(Playwright, browserChrome, ntag, chromeTag, pwKeepTags) || changed

		firefoxTag, err := m.getPWBrowserTag(browserFirefox, tag)
		if err != nil {
			return err
		}
		changed = m.updateTags(Playwright, browserFirefox, ntag, firefoxTag, pwKeepTags) || changed

		webkitTag, err := m.getPWBrowserTag(browserWebkit, tag)
		if err != nil {
			return err
		}
		changed = m.updateTags(Playwright, browserWebkit, ntag, webkitTag, pwKeepTags) || changed
